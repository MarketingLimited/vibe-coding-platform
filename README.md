...
`tools/build-project-images.sh` لضمان توفر صور المشاريع قبل تشغيل الخدمات مباشرة عبر `docker compose` لخدمات `api`, `project-manager`, `cleanup`, و`redis`. بعد تفعيل
...
```bash
bash tools/setup/deactivate.sh
```

سيقوم بإيقاف خدمات Docker Compose، مع خيار لحذف شبكة `vibe-network` والبيانات المؤقتة ضمن المسار الذي حددته في متغير `DATA_DIR`.
